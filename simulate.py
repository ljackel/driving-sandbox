...
from generate_world import (
    ...
    convoy_right_lane_active,
    convoy_right_lane_pass_weight,
    draw_bev_topdown_car,
    draw_convoy_right_lane_bots_bev,
...
def _ego_lateral_offset_px_at_y(
    ...
    *,
    ...
    slow_bot_y_arc_chain: list[float | None] | None = None,
) -> float:
    ...
    ``_world_bgr_slow_bot_layers`` / ``y_slow_bot_kinematics``). ``skip_slow_bot``: omit bot (cheap paths).
    ``slow_bot_y_arc_chain``: optional single-element list ``[y_bot_row]`` updated in-place to chain hints
    along the BEV trail (oldest sample first).
    """
    if on_ramp:
    ...
    if w_bot_override is not None:
        ...
        hint = None
        if slow_bot_y_arc_chain is not None and slow_bot_y_arc_chain:
            hint = slow_bot_y_arc_chain[0]
        w_bot, _, _, _, y_br = slow_bot_car_pass_blend_and_pose(
            float(y), dw, y_ref, r_lane, y_arc_hint=hint
        )
        if slow_bot_y_arc_chain is not None and y_br is not None:
            slow_bot_y_arc_chain[0] = float(y_br)
    if (
        bool(getattr(cfg, "BOT_CONVOY_ENABLE", False))
        and not on_ramp
        and world_bgr is not None
    ):
        y_ref_c = float(world_bgr.shape[0]) - float(cfg.DATASET_MAP_MARGIN)
        w_convoy = convoy_right_lane_pass_weight(
            float(y), dw, y_ref_c, float(dw.px_per_m)
        )
    w_left = max(float(w_bot), float(w_convoy))
    return lateral_offset_px_avoid_roadkill(
        ...
        extra_left_lane_blend=w_left,
        extra_merge_right_blend=0.0,
    )


def _y_main_slow_bot_kin_resolve(
    *,
    on_ramp: bool,
    ego_x: float,
    ego_y: float,
    ...
    margin: float,
    world_bgr: np.ndarray,
    main_y_fallback: float,
) -> float:
    """Main-centerline row for slow-bot / convoy kinematics while ego is on the Bézier (or ``ego_y`` on main)."""
    if not on_ramp:
        return float(ego_y)
    snap = _snap_main_right_lane_nearest(
        dw, float(ego_x), float(ego_y), h, margin, world_bgr
    )
    if snap is not None:
        return float(snap[1])
    return float(main_y_fallback)


def _world_bgr_slow_bot_layers(
    world_bgr: np.ndarray,
    dw: DrivingWorld,
    y: float,
    *,
    ...
    y_slow_bot_kinematics: float | None = None,
) -> tuple[np.ndarray, np.ndarray, float, float | None]:
    """Slow-bot pass blend; two map copies for FP warp vs BEV map display.

    ``y_slow_bot_kinematics``: when ego is on the off-ramp, main-road row driving bot arc motion (ghost
    forward on the centerline each step). If ``on_ramp`` and this is ``None``, falls back to ``y``.

    Returns ``(layer_warp, layer_map, w_bot, y_bot_row)``. ``y_bot_row`` is the solved centerline row
    for the bot (``None`` when off-map or disabled). When there is no bot, both layers equal ``world_bgr``.
    """
    y_pose = (
        float(y_slow_bot_kinematics)
        if (on_ramp and y_slow_bot_kinematics is not None)
        else float(y)
    )
    ...
    w_bot = 0.0
    y_br: float | None = None
    qx: float | None = None
    qy: float | None = None
    ppsi: float | None = None
    if bool(getattr(cfg, "BOT_CAR_ENABLE", False)):
        w_bot, qx, qy, ppsi, y_br = slow_bot_car_pass_blend_and_pose(
            float(y_pose), dw, y_ref, r_lane, y_arc_hint=y_arc_hint
        )
    has_convoy = convoy_right_lane_active()
    need_copy = (qx is not None) or has_convoy
    if not need_copy:
        return world_bgr, world_bgr, float(w_bot), y_br
    out_warp = world_bgr.copy()
    out_map = world_bgr.copy()
    if qx is not None and qy is not None and ppsi is not None:
        draw_slow_bot_car_bev(
            out_warp, qx, qy, ppsi, float(dw.px_per_m), for_perspective_warp=True
        )
        draw_slow_bot_car_bev(
            out_map, qx, qy, ppsi, float(dw.px_per_m), for_perspective_warp=False
        )
    if has_convoy:
        draw_convoy_right_lane_bots_bev(
            out_warp,
    ...
    return out_warp, out_map, float(w_bot), y_br
...
def _bev_nav_box_bottom_y(text: str, *, h_map: int, x0: int = 10, y0: int = 8) -> int:
    """Same vertical extent as ``_bev_draw_nav_instruction_box`` (no drawing)."""
    font = cv2.FONT_HERSHEY_SIMPLEX
    scale = 0.58
    thick = 2
    pad_x, pad_y = 10, 8
...
def _bev_overlay_step_hud(
    ...
    *,
    ...
    line1 = f"step {step_idx}   Space pause   q quit   Ctrl+C (terminal)"
    cv2.putText(
        vis,
        line1,
        (10, line1_y),
...
def _trail_overlay_xy_for_path_index(
    dw: DrivingWorld,
    world_bgr: np.ndarray,
    ...
    *,
    trail_slow_bot: bool,
    arc_hint_chain: list[float | None] | None,
) -> tuple[float, float]:
    """Right-lane BEV vertex for ``path[index]`` (trail polyline point)."""
    xc, yc, _, on_r, ur, rid = path[index]
    ...
    lat_i = _ego_lateral_offset_px_at_y(
        ...
        float(psi_d),
        on_ramp=bool(on_r),
        skip_slow_bot=not trail_slow_bot,
        slow_bot_y_arc_chain=(
            arc_hint_chain if trail_slow_bot and not on_r else None
        ),
    )
...
def _bev_realtime_frame(
    ...
    *,
    ...
    trail_overlay_qxy: list[tuple[float, float]] | None = None,
) -> np.ndarray:
    """Bird's-eye frame: trail and ego in the **right lane** (not centerline).

    ``world_bgr`` is the base map (no bot); slow-bot art uses ``y`` on the main road or
    ``y_slow_bot_kinematics`` when ``on_ramp`` (ghost main-row motion).
    Trail lateral uses ``world_bgr`` for roadkill and the same slow-bot blend as the ego (per path point).

    **Performance:** with ``trail_overlay_qxy`` (one precomputed right-lane ``(x,y)`` per path index, built
    incrementally in the sim loop), only **subsampling** and ``polylines`` run here—O(``SIM_BEV_TRAIL_MAX_POINTS``).
    Without it, trail vertices are recomputed each frame (expensive).
    ...
    if bot_visual_base is not None:
        ...
        bot_layer = _world_bgr_slow_bot_layers(
            world_bgr,
            dw,
            float(y),
            on_ramp=on_ramp,
            map_h=map_h,
            y_arc_hint=None,
            y_slow_bot_kinematics=y_slow_bot_kinematics,
        )[1]
    ...
    if len(path) >= 2:
        ...
        last_i = n - 1
        num_samples = min(n, max_pts)
        raw = np.linspace(0.0, float(last_i), num=num_samples, dtype=np.float64)
        idxs = sorted({int(round(float(x))) for x in raw})
        if idxs[0] != 0:
            idxs.insert(0, 0)
        if idxs[-1] != last_i:
            idxs.append(last_i)
        ...
        use_cache = (
            trail_overlay_qxy is not None and len(trail_overlay_qxy) == n
        )
        if use_cache:
            for j, ii in enumerate(idxs):
                qx, qy = trail_overlay_qxy[ii]
                qs[j, 0] = qx
                qs[j, 1] = qy
        else:
            trail_slow_bot = bool(
                getattr(cfg, "SIM_BEV_TRAIL_INCLUDE_SLOW_BOT", True)
            )
            trail_y_chain: list[float | None] = [None]
            for j, i in enumerate(idxs):
                ...
                lat_i = _ego_lateral_offset_px_at_y(
                    ...
                    skip_slow_bot=not trail_slow_bot,
                    slow_bot_y_arc_chain=(
                        trail_y_chain if trail_slow_bot and not on_r else None
                    ),
                )
                qx, qy = _right_lane_overlay_xy(float(xc), float(yc), psi_d, lat_i)
                qs[j, 0] = qx
...
def run_simulation() -> tuple[
    ...
    **Off-ramps:** With projection and ``OFFRAMP_ENABLE``, the pose advances on the main spline until a
    ...
    x, y = x0, y0
    ego_speed_ms = float(cfg.SIM_SPEED_M_S)
    if convoy_right_lane_active():
        alt = getattr(cfg, "BOT_CONVOY_EGO_SPEED_M_S", None)
        if alt is not None:
            ego_speed_ms = float(alt)
    base_step_dist_px = ego_speed_ms * cfg.SIM_DT * px_per_m
    last_slow_bot_y_row: float | None = None
    ...
    trail_overlay_qxy: list[tuple[float, float]] = []
    trail_trail_arc_chain: list[float | None] = [None]
    trail_slow_bot = bool(getattr(cfg, "SIM_BEV_TRAIL_INCLUDE_SLOW_BOT", True))
    trail_overlay_qxy.append(
        _trail_overlay_xy_for_path_index(
            dw,
            world,
            path,
            0,
            trail_slow_bot=trail_slow_bot,
            arc_hint_chain=trail_trail_arc_chain,
        )
    )
    if on_ramp:
        snap0 = _snap_main_right_lane_nearest(
            dw, float(x), float(y), h, margin, world
        )
        if snap0 is not None:
            y_main_slow_bot_kin = float(snap0[1])
        elif ramp_id < len(off_branch_ys):
            y_main_slow_bot_kin = float(off_branch_ys[ramp_id])
        else:
            y_main_slow_bot_kin = float(y)
    else:
        y_main_slow_bot_kin = float(y)
    ...
    try:
        ...
        for sim_i in range(cfg.SIM_MAX_STEPS):
            ...
            def _print_sim_step_timing(note: str = "") -> None:
                dt_ms = (time.perf_counter() - t_step_begin) * 1000.0
                extra = f"  [{note}]" if note else ""
                print(
                    f"sim step {sim_i}  wall {dt_ms:.2f} ms  sim_t {t_sim:.3f} s  "
                    f"|path| {len(path)}{extra}",
                    flush=True,
                )
            ...
            if on_ramp:
                ...
                if ev_pose is None:
                    _print_sim_step_timing("exit: offramp ev_pose None")
                    break
                _, (fx, fy), _ = ev_pose
                psi_cam = float(np.arctan2(fy, fx))
            else:
                psi_cam = initial_heading_road_aligned(dw.cs, y)
            world_cam_warp, _, w_bot_fp, y_br_pre = _world_bgr_slow_bot_layers(
                world,
                dw,
                float(y),
                on_ramp=on_ramp,
                map_h=h,
                y_arc_hint=last_slow_bot_y_row,
                y_slow_bot_kinematics=(
                    float(y_main_slow_bot_kin) if on_ramp else None
                ),
            )
            view = get_view_from_pose(
                ...
                _ego_lateral_offset_px_at_y(
                    ...
                    float(psi_cam),
                    on_ramp=on_ramp,
                    w_bot_override=(None if on_ramp else w_bot_fp),
                ),
            )
            if view is None:
                _print_sim_step_timing("exit: get_view_from_pose None")
                break
            ...
            if cfg.SIM_PROJECT_REF_ONTO_MAIN_ROAD:
                if ramp_kinematics and on_ramp:
                    ...
                    if step_r is None:
                        _print_sim_step_timing("exit: offramp_step_arc_px None")
                        break
                    u_ramp, x, y = step_r
            ...
            if not on_ramp:
                y_main_slow_bot_kin = float(y)
            elif not ramp_merge_this_step:
                _, y_main_slow_bot_kin = _advance_main_centerline_arc_px(
                    dw,
                    y_main_slow_bot_kin,
                    step_dist_px,
                    margin=margin,
            ...
            world_cam_warp_post, world_cam_map_post, w_bot_post, y_br_post = (
                _world_bgr_slow_bot_layers(
                    world,
                    dw,
                    float(y),
                    on_ramp=on_ramp,
                    ...
                    y_arc_hint=(
                        y_br_pre
                        if y_br_pre is not None
                        else last_slow_bot_y_row
                    ),
                    y_slow_bot_kinematics=(
                        float(y_main_slow_bot_kin) if on_ramp else None
                    ),
            ...
            if y_br_post is not None:
                last_slow_bot_y_row = float(y_br_post)
            lat_hit = _ego_lateral_offset_px_at_y(
                ...
                float(psi_draw),
                on_ramp=on_ramp,
                w_bot_override=(None if on_ramp else w_bot_post),
            )
            ...
            bev_fr: np.ndarray | None = None
            want_bev_mp4 = bev_video_abs is not None
            bev_hud_on_shared_frame = live_bev and not want_bev_mp4
            if want_bev_mp4 or live_bev:
                bev_hint_shared = (
                    "PAUSED: drag ego car to move, release on road"
                    if (bev_hud_on_shared_frame and rt_sim_paused)
                    else None
                )
                bev_fr = _bev_realtime_frame(
                    ...
                    on_ramp=on_ramp,
                    map_h=h,
                    bot_visual_base=world_cam_map_post,
                    y_slow_bot_kinematics=(
                        float(y_main_slow_bot_kin) if on_ramp else None
                    ),
                    extra_hint=bev_hint_shared,
                    nav_exit_active=_bev_nav_exit_active(float(y), h),
                    nav_exit_text=str(cfg.SIM_NAV_EXIT_INSTRUCTION_TEXT),
                    show_step_hud=bev_hud_on_shared_frame,
                    trail_overlay_qxy=trail_overlay_qxy,
                )
            ...
            if live_bev or live_drv:
                try:
                    ...
                    if view_live is None:
                        _print_sim_step_timing("exit: view_live None")
                        break
                    ...
                    if live_bev:
                        ...
                        if bev_fr is None:
                            ...
                            fr = bev_fr.copy()
                        if want_bev_mp4:
                            nav_active = _bev_nav_exit_active(float(y), h)
                            nav_txt = str(cfg.SIM_NAV_EXIT_INSTRUCTION_TEXT)
                            below_nav = (
                                _bev_nav_box_bottom_y(nav_txt, h_map=h)
                                if (nav_active and nav_txt)
                                else 0
                            )
                            pause_hint = (
                                "PAUSED: drag ego car to move, release on road"
                    ...
                    while True:
                        ...
                        if rp is not None:
                            ...
                            trail_overlay_qxy.clear()
                            trail_trail_arc_chain[0] = None
                            trail_overlay_qxy.append(
                                _trail_overlay_xy_for_path_index(
                                    dw,
                                    world,
                            ...
                            fb_reloc = (
                                float(off_branch_ys[ramp_id])
                                if (
                                    on_ramp
                                    and ramp_id < len(off_branch_ys)
                                )
                                else float(y)
                            )
                            y_sb_reloc = _y_main_slow_bot_kin_resolve(
                                on_ramp=on_ramp,
                                ego_x=float(x),
                                ego_y=float(y),
                                dw=dw,
                                ...
                                margin=margin,
                                world_bgr=world,
                                main_y_fallback=fb_reloc,
                            )
                            if on_ramp:
                                y_main_slow_bot_kin = float(y_sb_reloc)
                            (
                                world_cam_warp_post,
                                world_cam_map_post,
                                w_bot_post,
                                ...
                                y_arc_hint=last_slow_bot_y_row,
                                y_slow_bot_kinematics=(
                                    float(y_sb_reloc) if on_ramp else None
                                ),
                            )
                            if y_br_reloc is not None:
                                last_slow_bot_y_row = float(y_br_reloc)
                            lat_hit = _ego_lateral_offset_px_at_y(
                                dw,
                                float(y),
                                world,
                                ...
                                on_ramp=on_ramp,
                                w_bot_override=(
                                    None if on_ramp else w_bot_post
                                ),
                            )
                            qx_hit, qy_hit = _right_lane_overlay_xy(
                                float(x), float(y), float(psi_draw), lat_hit
                        ...
                        if paused and isinstance(prv, dict) and rt_ui.get("ego_drag"):
                            ...
                            y_sb_drag = _y_main_slow_bot_kin_resolve(
                                ...
                                margin=margin,
                                world_bgr=world,
                                main_y_fallback=float(y_main_slow_bot_kin),
                            )
                            _, disp_bev_base, w_bot_pv, _y_br_pv = (
                                _world_bgr_slow_bot_layers(
                                    world,
                                    dw,
                                    float(disp_y),
                            ...
                            y_sb_bev = (
                                float(y_sb_drag) if on_ramp_for_bev else None
                            )
                        elif fr is not None:
                            disp_bev_base = world_cam_map_post
                        if fr is not None:
                            bh = (
                                "PAUSED: drag ego car to move, release on road"
                                if paused
                            ...
                            fr = _bev_realtime_frame(
                                ...
                                y_slow_bot_kinematics=y_sb_bev,
                                extra_hint=bh,
                                nav_exit_active=_bev_nav_exit_active(
                                    float(disp_y), h
                                ),
                        ...
                        continue
                    if user_quit_rt:
                        _print_sim_step_timing("quit: realtime UI")
                        break
    ...
    if video_writer is not None:
        if on_ramp:
            ...
            psi_end = initial_heading_road_aligned(dw.cs, y)
        world_end_warp, _, w_bot_end, _ = _world_bgr_slow_bot_layers(
            world,
            dw,
            float(y),
            on_ramp=on_ramp,
        ...
        view_end = get_view_from_pose(
            ...
            _ego_lateral_offset_px_at_y(
                ...
                float(psi_end),
                on_ramp=on_ramp,
                w_bot_override=(None if on_ramp else w_bot_end),
            ),
        )
...
def _open_path_default_app(path: str) -> None:
    ...
    try:
        ...
        print(
            f"Note: could not open {path!r} ({exc}). Play the file manually.",
            file=sys.stderr,
        )
...
def main() -> None:
    ...
    if path:
        ...
        _, world_bot_l, w_bot_l, _ = _world_bgr_slow_bot_layers(
            ...
            y_arc_hint=None,
            y_slow_bot_kinematics=(
                float(y_main_slow_bot_kin_final) if onl else None
            ),
        )
        lat_l = _ego_lateral_offset_px_at_y(
            ...
            float(psi_draw_l),
            on_ramp=bool(onl),
            w_bot_override=(None if onl else w_bot_l),
        )
        trail_arc_m: list[float | None] = [None]
        trail_slow_m = bool(getattr(cfg, "SIM_BEV_TRAIL_INCLUDE_SLOW_BOT", True))
        for i in range(len(path)):
            trail_qxy_m.append(
                _trail_overlay_xy_for_path_index(
                    ...
                    path,
                    i,
                    trail_slow_bot=trail_slow_m,
                    arc_hint_chain=trail_arc_m,
                )
            )
    ...
    # Same right-lane footprint as ``sim_bev_path.png`` (roadkill + optional slow-bot); do not use
    # ``skip_slow_bot`` chord smoothing here.
    if len(trail_qxy_m) >= 2:
        arr = np.asarray(trail_qxy_m, dtype=np.float32)
        arr_i = np.round(arr).astype(np.int32).reshape(-1, 1, 2)
        cv2.polylines(vis, [arr_i], False, (0, 0, 255), 2, cv2.LINE_AA)
    if len(path) > 0 and len(trail_qxy_m) == len(path):
        ...
        sq0, sq1 = trail_qxy_m[0]
        eq0, eq1 = trail_qxy_m[-1]
        _draw_bev_ego_car_icon(
            vis, float(sq0), float(sq1), ps0, float(dw_overlay.px_per_m)
        )
        _draw_bev_ego_car_icon(
            vis,
            float(eq0),
    ...
    if bev_video and bool(getattr(cfg, "SIM_PLAY_BEV_MP4_WHEN_DONE", True)):
        _open_path_default_app(bev_video)
